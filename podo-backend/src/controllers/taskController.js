...
function taskController(pool) {
  async function createTask(req, res) {
    try {
      ...
      // questions JSONB olarak saklanıyor — parse edilmiş haliyle döndür
      return res.status(201).json({
        ...task,
        questions: typeof task.questions === 'string' ? JSON.parse(task.questions) : task.questions,
      });
    } catch (err) {
      console.error("[taskController] createTask hatası:", err.message || err);
      return res.status(500).json({ error: "Görev oluşturulurken hata oluştu", detail: err.message });
    }
  }
